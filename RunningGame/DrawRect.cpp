...
DrawRect::DrawRect(LPDIRECT3DDEVICE9 device, POINT coord, POINT size, DWORD color)
	: device(device)
	, lineColor(color)
	, coord(coord)
	, size(size)
...
void DrawRect::SetCoord(POINT coord)
{
	this->coord = coord;
