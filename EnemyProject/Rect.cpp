...
Rect::Rect(LPDIRECT3DDEVICE9 device, D3DXVECTOR2 coord,
	D3DXVECTOR2 size, DWORD color)
	: device(device)
	, lineColor(color)
	, coord(coord)
	, size(size)
...
RECT Rect::GetRect()
{
	RECT rect = { coord.x, coord.y, coord.x + size.x, coord.y + size.y };
...
void Rect::SetScale(const D3DXVECTOR2 * scale)
{
	...
	//size.x = this->size.x * scale->x;
	//size.y = this->size.y * scale->y;
