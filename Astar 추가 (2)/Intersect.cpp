...
bool Intersect::IsPointInCircle(const POINT* point1, const POINT* point2, 
...
bool Intersect::IsContainRect(RECT* rect, Rect* comp1, Rect* comp2)
{
...
bool Intersect::IsPointinRect(POINT * pos, Rect * comp)
{
	bool isVertical = false;
	bool isHorizontal = false;

	RECT* rect1 = &comp->GetRect();

	if (rect1->left <= pos->x && rect1->right >= pos->y)
	{
		isHorizontal = true;
	...
	if (rect1->top <= pos->y && rect1->bottom >= pos->y)
	{
		isVertical = true;
